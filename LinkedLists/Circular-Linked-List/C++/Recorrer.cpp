...
void traverse(Node *head){
    //Lo primero que tenemos que hacer, es guardar la referencia del primer nodo, ya que pararemos de recorrer cuando volvamos a este
    Node *first = head->next;
    do{
        //Recorremos con la cabeza la lista
        head = head->next;
        cout<<head->data<<" ";
    }while(head->next != first); //Hasta que la el nodo, no apunte de nuevo al primer nodo de la lista
}
